...
function createJet(name) {
    ...
    jet.rate = new Proton.Rate(new Proton.Span(1, 20), new Proton.Span(0.01));
    ...
    jet.addInitialize(new Proton.Life(0, 2.5)); // lifecycle length -> start and end. good for smoketrails possibly, if long life
    jet.addInitialize(new Proton.V(20, new Proton.Vector3D(0, 1, 0), 180)); // radius, vector (), tha (something angle??)
    // not having a velocity looks like a good ray

    //addBehaviour
    jet.addBehaviour(new Proton.Alpha(0.75, 0));
    jet.addBehaviour(new Proton.Scale(.125, 2));
    jet.addBehaviour(new Proton.Force(0, -2.5, 0)); // fx, fy, life (this pushes upwards or downward)

    var color1 = new THREE.Color("rgb(250, 250, 250)");
